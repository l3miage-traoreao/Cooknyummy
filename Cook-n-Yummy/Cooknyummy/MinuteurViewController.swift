...
class MinuteurViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    //view not used
    ...
    @IBAction func activerMinuteur(_ sender:  UIButton) {
        compteur()
    }
    
    @IBOutlet weak var resetMinuteurBtn: UIButton!
    @IBAction func restMinuteurAction(_ sender: Any) {
        resetCompteur()
    }
    ...
    func compteur() {
        ...
        if (!estActif) {
            ...
            timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(MinuteurViewController.incrementer), userInfo: nil, repeats: true)
            timer.fire()
